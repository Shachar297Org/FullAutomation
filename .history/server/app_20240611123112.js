        ...
        for(let group = 0; group < groups.length; group++) {
            projectPromises.push(
                gitlabModule.getAllProjects(groups[group].id).then(projects => {
                    ...
                    for(let project = 0; project < projects.length; project++) {
                            gitlabModule.getAllIssues(projects[project].id).then(issues => {
                                ...
                                for(let issue = 0; issue < issues.length; issue++) {
                                    createdIssuePromise.push(githubModule.createGithubIssue(issue, groups[group].path, projects[project].name))
                                }
                                return Promise.all(createdIssuePromise);
                            })
                    }
                    ...
                    console.error("Error During Issue Migration ", e)
                })
            )
        }
    })
